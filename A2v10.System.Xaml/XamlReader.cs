...
{
	private readonly XamlNode _root = new("Root");
	...
	void ReadNode(NodeBuilder builder)
	{
		switch (_rdr.NodeType)
		{
			case XmlNodeType.Element:
				{
					var node = new XamlNode(_rdr.Name);
					StartNode(node);
	...
	{
		_xamlServiceProvider.AddService<T>(service ?? throw new ArgumentNullException(nameof(service)));
	}
